...
mon <- spTransform(mon, CRS("+init=epsg:4269 +proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"))
mon_buffer <- spTransform(mon_buffer, CRS("+init=epsg:4269 +proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"))

mon_df <- broom::tidy(mon)
mon_buffer_df <- broom::tidy(mon_buffer)
...
mon_buffer <- spTransform(mon_buffer, CRS("+init=epsg:4269"))
...
get_buffer_mons <- function(CSVFILE) {
  ...
  buffer_mons <- raster::intersect(csv_dat, mon_buffer) %>% 
    as.data.frame() %>% 
...
write_csv(buffer_mons_df, "data/mon_nf_air_monitors_annual.csv")
...
o3_pm_dat <- read_csv("data/mon_nf_air_monitors_annual.csv") %>% 
...
ggsave("figures/mon__nf_ozone_3_year_mean.jpg",
       height = 3,
       width = 4.2,
       unit = "in")
...
ggsave("figures/mon_nf_pm_3_year_mean.jpg",
       height = 3,
       width = 4.2,
       unit = "in")
...
ggsave("figures/mon_nf_pm_3_year_98th.jpg",
       height = 3,
       width = 4.2,
       unit = "in")
...
ggplot() +
  geom_polygon(aes(long, lat, group = group, alpha = "100km buffer"),
               color = "black",
               size = 1,
               fill = NA,
               data = mon_buffer_df) +
  geom_polygon(aes(long, lat, group = group, alpha = "Monongahela NF"),
               color = "darkgreen",
               size = 1,
               fill = "darkgreen",
               data = mon_df) +
...
ggsave("figures/mon_nf_oz_pm_monitors.jpg",
       height = 3.2,
       width = 4,
       units = "in")
